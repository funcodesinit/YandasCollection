...
import { fetchPublicOrder } from '@/store/actions/cartActions'
                  ...
                  <div className="flex items-center border-b border-gray-200 p-4 sm:grid sm:grid-cols-4 sm:gap-x-6 sm:p-6">
                    <dl className="grid flex-1 grid-cols-2 gap-x-6 text-sm sm:col-span-4 sm:grid-cols-4 lg:col-span-2">
                      ...
                      <div>
                        ...
                        <dd className="mt-1 font-medium text-gray-900 text-bold">ZMW {order.total}</dd>
                      </div>

                      <div>
                        <dt className="font-medium text-gray-900">status</dt>
                        <dd className="mt-1 font-medium text-gray-900 capitalize">
                          {order.status === "PENDING" ? (<span className="inline-flex items-center gap-x-1.5 rounded-md bg-yellow-100 px-2 py-1 text-xs font-medium text-yellow-700">
                          ...
                          {order.status === "SHIPPED" ? (<span className="inline-flex items-center gap-x-1.5 rounded-md bg-indigo-100 px-2 py-1 text-xs font-medium text-indigo-700">
                          ...
                          {order.status === "DELIVERED" ? (<span className="inline-flex items-center gap-x-1.5 rounded-md bg-green-100 px-2 py-1 text-xs font-medium text-green-700">
                          ...
                          {order.status === "RETURNED" ? (<span className="inline-flex items-center gap-x-1.5 rounded-md bg-pink-100 px-2 py-1 text-xs font-medium text-pink-700">
                          ...
                          {order.status === "CANCELLED" ? (<span className="inline-flex items-center gap-x-1.5 rounded-md bg-red-100 px-2 py-1 text-xs font-medium text-red-700">
                        ...
                        <div className="flex items-center sm:items-start">
                          ...
                          <div className="ml-6 flex-1 text-sm">
                            <div className="font-medium text-gray-900 sm:flex sm:justify-between">
                              <div className='capitalize text-lg'>{product?.cartItem?.product?.name}</div>
                              <div className="">
                                <a
                            ...
                            <p className="mt-2 sm:mt-0 text-md capitalize">unit price: K{product?.cartItem?.product?.price}</p>
                            <p className="hidden text-gray-500 sm:mt-2 sm:block">size: {product?.cartItem?.size.size} ({product?.cartItem?.size.range})</p>
                            <p className="hidden text-gray-500 sm:mt-2 sm:block">quantity: {product?.cartItem?.quantity} units</p>
                            <p className="hidden text-gray-500 sm:mt-2 sm:block">total Price: K{product?.cartItem?.quantity * product?.cartItem?.product?.price}</p>
